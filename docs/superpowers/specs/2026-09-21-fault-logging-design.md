...
- 抽取块失败不再只写「抽取失败」四个字：日志打 LiteLLM/模型原始异常栈；作业 `error` 为可读中文（`public_llm_message`）。若原始异常像堆栈（含 `Traceback` 或 `File "`）、LiteLLM/异常类名，或 HTML（`rel=icon`、`data:image`、标签），对用户改为「模型调用失败」。不得把 `str(exc)` 截断后原样返回界面。设置页拉取模型、测联通同样走该映射。
...
tips、作业状态文案只有友好中文。成功仍为主色左边线。拉取模型 / 测联通失败也走同一套映射，不把供应商异常页 HTML 或 LiteLLM 类名送进 tips。`ApiError` 与 `tipText` 再过滤一次，防止非 JSON 错误页漏到界面。
